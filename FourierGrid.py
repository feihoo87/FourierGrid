...
@author: feihoo87
"""
__all__ = ['FourierGrid', 'QuantumSystem', 'Noise', 'wigner']
...
class FourierGrid(object):
    ...
    def _check_size_and_bound(self, dims, size, bound):
        if dims == 1:
            ...
            for b in bound:
                if len(b) != 2:
                    raise Exception('all length of elements in bound must be 2')
    ...
    def _init_step(self):
        if self.dims == 1:
            ...
            self.x_step = [(self.bound[i][1]-self.bound[i][0])/self.size[i] for i in range(self.dims)]
            self.k_step = [2*np.pi/(self.x_step[i]*self.size[i]) for i in range(self.dims)]
    ...
    def x(self, indexing='xy', no_grid=False, sparse=True):
        ...
        x = [np.linspace(bound[i][0], bound[i][1], size[i]) for i in range(self.dims)]
        if no_grid:
            return x
        else:
    ...
    def k(self, indexing='xy', no_grid=False, fft_shift=False, sparse=True):
        size = [self.size] if self.dims == 1 else self.size
        k = [np.linspace(-(size[i]-1)/2, (size[i]-1)/2, size[i])*self.k_step[i] for i in range(self.dims)]
        if fft_shift:
    ...
    def _make_T(self):
        ...
        shape = [self.size, self.size] if self.dims == 1 else list(self.size) * 2
    ...
    def U(self, U_func, clip=True, vmin=None, vmax=None, indexing='xy', time=None):
    ...
    def H(self, U_func, time_dependent=False, time=None):
        '''Hamiltonian
        '''
        U = self.U(U_func, indexing='ij', clip=False, time=(time if time_dependent else None)).flatten()
    ...
    def E(self, U):
        ...
        return eigh(H, b=None, lower=True, eigvals_only=True,
                ...
                check_finite=False)
        '''
        w, v = eigh(self.H(U), b=None, lower=True, eigvals_only=False, overwrite_a=True,
    ...
    def States(self, U):
        ...
        v : {(..., M, M) ndarray, (..., M, M) matrix}
            ...
            object.
        '''
        w, v = eigh(self.H(U), overwrite_a=True, overwrite_b=True, check_finite=False)
        self.__H_distroyed = True
        return w, v
...
class QuantumSystem(object):
    ...
    def Dynamic(self, psi0, tlist, time_dependent=False, with_U=False):
        ...
        def func(t, psi):
            ...
            return dpsi
        sol = ode(func).set_integrator('zvode').set_initial_value(psi0, tlist[0])
        ...
        if with_U:
            U_list = [self.U(*self.grid.x(), **dict(time=t)) for t in tlist[:len(ret)]]
...
class Noise(object):
    def __init__(self, max_time, max_freq, amp=1e-4, T=0.01, seed=None):
        ...
        def planck(f, T):
            return (2*const.h*f**3/const.c**2)/(np.exp(const.h*f/const.k/T)-1)
...
def wigner(psi):
    '''Wigner function
                 1             +Inf    *
    P(x, p) = ------- Integrate     psi (x+y) psi(x-y) exp(2i p y / hbar) dy
              pi hbar          -Inf
    '''
    N = len(psi)
    _wig = np.zeros((N, N))
    pp = np.convolve(psi.conj(), psi)
    #_psi = np.zeros(3*N, dtype=complex)
    #_psi[N:2*N] = psi
    x = np.linspace(1, N, N)
    x -= x.mean()

    x, k = np.meshgrid(x, k)
    e = np.exp(2j*np.pi*x*k/N)
